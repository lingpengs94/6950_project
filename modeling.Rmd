...
```{r}
model_diag_quart = augment(fit.BIC.quart, fifa_model)
par(mfrow=c(1,3))
qqnorm(fit.BIC.quart$residuals,
       xlab="Normal quantiles", ylab="Sample quantiles", main="")
qqline(fit.BIC.quart$residuals)
hist(fit.BIC.quart$residuals, xlab="Residuals", main="")
plot(y=model_diag_quart %>% pull(.resid) , x=model_diag_quart %>% pull(.fitted),ylim = c(-1.5,1.5),xlab="Residual", ylab="Fitted")
abline(h=0,col="red")
```
