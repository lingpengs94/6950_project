...
fifa$team_position <- rockchalk::combineLevels(fifa$team_position,levs = c("CB","LB","LCB","LWB","RB","RCB","RWB","SUB"),
                                      newLabel = c("Defender") )
fifa$team_position <- rockchalk::combineLevels(fifa$team_position,levs = c("CAM","CDM","CM","LAM","LCM","LDM","RAM","RCM","RDM","RM","LM"),
                                      newLabel = c("Midfielder") )
fifa$team_position <- rockchalk::combineLevels(fifa$team_position,levs = c("CF","LF","LS","LW","RES","RF","RS","RW","ST"),
                                      newLabel = c("Forward") )
...
According to the density plot (Figure 1) of the response (value in Euro), the original data is heavily right skewed, while the distribution after log-transformation is more likely to Normal. Thus, log transformed value in Euro is our reponse.

The correlation plot (Figure 2) is made to give us a general idea how continous predictors are related to each other. Shooting, passing, and dribbling are related and they might reflact the simlar aspects of player.

From the plot matrix (Figure 3), the ditribtions of all continuous variables are roughly Normal. The overall scores and log-value in Euro are highly correlated (0.972), suggesting that overall scores is a very important predictors. Besides, the correlation between passing and dribblingis over 0.8, indicating that there might be some potential collinearity.

From the box plots of catergorical variables(Figure 4), the distributions for preferred foot and league name are quite symmetric with similar variance, while the variance for forward level in team position is a slightly larger than others.
